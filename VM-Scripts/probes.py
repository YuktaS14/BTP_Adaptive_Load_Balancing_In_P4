...
def get_system_metrics(in_bytes_prev, out_bytes_prev, time_prev):
    ...
    in_util = ((in_bytes_recv - in_bytes_prev) * 8) / (time_diff * LINESPEED*1024*1024) * 100
    out_util = ((out_bytes_sent - out_bytes_prev) * 8) / (time_diff * LINESPEED*1024*1024) * 100

    '''
    print("In_util: ", 8*(in_bytes_recv))
    print("In_util: ", 8*(in_bytes_prev))
    print("Out_util: ", 8*(out_bytes_sent - out_bytes_prev))
    '''

    in_util_left = int((100 - in_util) / 10)
    out_util_left = int((100 - out_util) / 10)

    link_util = out_util_left

    '''
    throughput = (in_bytes_recv - in_bytes_prev + out_bytes_sent - out_bytes_prev)*8
    '''

    print(cpu_percent, memory_percent, link_util)
    return cpu_percent, memory_percent, link_util, in_bytes_recv, out_bytes_sent
...
if __name__ == "__main__":
    ...
    try:
        while time.time() < end_time:
            cpu_percent, memory_percent, link_util, in_bytes_prev, out_bytes_prev = get_system_metrics(in_bytes_prev, out_bytes_prev,
                                                                                    time_prev)
            #print("thr-time : ", throughput, " ", time_diff)
            #val = throughput / time_diff
            #print("Throughput at time :", time_prev, " = ", val , "bits/s")
        ...
        #print("Average Throughput:", (average_throughput/1024), "Kb/s")
